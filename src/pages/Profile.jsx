...
export default function Profile() {
  ...
  return ( !user ? <Loading />
    : <div  id="feed-scroll" className="max-h-screen max-w-6xl mx-auto flex flex-col overflow-y-scroll no-scrollbar">
      ...
      {/* Posts Media Likes */}
      <div className=" flex flex-col flex-1 max-w-6xl m-3">
          {/* Buttons */}
          <div className="w-full flex flex-wrap max-md:flex-col gap-0 sm:gap-2 justify-center items-center">
              <button onClick={()=>setSelectedBtn("posts")} className={`${selectedBtn === "posts" ? "bg-indigo-500 text-white" : "bg-slate-200 text-gray-700" } shadow cursor-pointer py-2 w-85 sm:w-45 md:w-45 flex items-center justify-center max-sm:rounded-t-2xl sm:rounded-md text-xl`}>Posts</button>
              <button onClick={()=>setSelectedBtn("media")} className={`${selectedBtn === "media" ? "bg-indigo-500 text-white" : "bg-slate-200 text-gray-700" } shadow cursor-pointer py-2 w-85 sm:w-45 md:w-45 flex items-center justify-center sm:rounded-md text-xl`}>Media</button>
              <button onClick={()=>setSelectedBtn("likes")} className={`${selectedBtn === "likes" ? "bg-indigo-500 text-white" : "bg-slate-200 text-gray-700" } shadow cursor-pointer py-2 w-85 sm:w-45 md:w-45 flex items-center justify-center max-sm:rounded-b-2xl sm:rounded-md text-xl`}>Likes</button>
          </div>
